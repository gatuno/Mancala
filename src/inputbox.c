...
InputBox *crear_inputbox (InputBoxFunc func, const char *ask, const char *text, InputBoxFunc close_func) {
	...
	if (strcmp (ib->buffer, "") != 0) {
		ib->text_s = TTF_RenderUTF8_Blended (ttf16_burbank_small, ib->buffer, blanco);
	} else {
		ib->text_s = NULL;
	}
	
	if (strcmp (ask, "") == 0) {
		ib->text_ask = TTF_RenderUTF8_Blended (ttf16_burbank_small, _("(No text)"), blanco);
	} else {
		ib->text_ask = TTF_RenderUTF8_Blended (ttf16_burbank_small, ask, blanco);
	}
